...
setup_conv = ConversationHandler(
    ...
    states={
        SETUP_STORE_NAME: [MessageHandler(filters.TEXT & ~filters.COMMAND, setup_store_name)],
        SETUP_DAILY_PLAN: [MessageHandler(filters.TEXT & ~filters.COMMAND, setup_daily_plan)],
        SETUP_MONTHLY_ACQUIRING_PLAN: [MessageHandler(filters.TEXT & ~filters.COMMAND, setup_monthly_acquiring_plan)],
        SETUP_ACQUIRING_BASE: [MessageHandler(filters.TEXT & ~filters.COMMAND, setup_acquiring_base)],
        SETUP_REPORT_TIME: [MessageHandler(filters.TEXT & ~filters.COMMAND, setup_report_time)],
        SETUP_REPORT_CHAT_ID: [MessageHandler(filters.TEXT & ~filters.COMMAND, setup_report_chat_id)],
        SETUP_BOSS_ID: [MessageHandler(filters.TEXT & ~filters.COMMAND, setup_boss_id)],
        SETUP_CONFIRM: [CallbackQueryHandler(confirm_setup_store, pattern="^(confirm_setup_store|cancel_setup_store)$")],
    },
    fallbacks=[CommandHandler("cancel", cancel)],
    per_message=False,
)
