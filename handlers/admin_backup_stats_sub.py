...
async def set_full_report_chat_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.message.reply_text(
        "Введи chat_id группы или человека,\n"
        "куда отправлять полный дубль отчёта."
    )

    return SET_FULL_REPORT_CHAT

async def admin_full_report_chat_entry(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    ...
    if not stores:
        await query.message.reply_text("У вас нет магазинов для настройки дубля.")
        return ConversationHandler.END
    ...
    if len(stores) == 1:
        store = stores[0]
        context.user_data["full_report_store_id"] = store["id"]
        context.user_data["full_report_store_name"] = store["name"]

        await query.message.reply_text(
            f"Магазин: {store['name']}\n\n"
            "Введите chat_id, куда отправлять полный дубль отчёта.\n\n"
            "Можно указать:\n"
            "— ID вашей лички\n"
            "— ID локальной группы магазина\n\n"
            "Чтобы отключить дубль, введите 0."
        )
        return SET_FULL_REPORT_CHAT_VALUE

    await query.message.reply_text(
        "Выбери магазин для настройки дубля:",
        reply_markup=stores_keyboard(stores, "admin_full_report_store")
    )

    return SET_FULL_REPORT_CHAT_STORE


async def admin_select_full_report_store(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    ...
    store_id = int(query.data.replace("admin_full_report_store_", "", 1))
    user_id = update.effective_user.id
    stores = get_user_stores(user_id)
    ...
    context.user_data["full_report_store_id"] = selected_store["id"]
    context.user_data["full_report_store_name"] = selected_store["name"]
...
async def admin_save_full_report_chat(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = update.message.text.strip()
    ...
    try:
        ...
        await update.message.reply_text(
            ...
            "Или 0, чтобы отключить дубль."
        )
        return SET_FULL_REPORT_CHAT_VALUE

    store_id = context.user_data.get("full_report_store_id")
    store_name = context.user_data.get("full_report_store_name")

    if not store_id:
        await update.message.reply_text("Магазин потерялся. Начни заново через /start.")
        return ConversationHandler.END

    value = None if chat_id == 0 else chat_id

    update_full_report_chat_id(store_id, value)

    context.user_data.clear()

    if value is None:
        await update.message.reply_text(
            f"✅ Дубль полного отчёта отключён.\n\n"
            f"Магазин: {store_name}"
        )
    else:
        await update.message.reply_text(
            f"✅ Дубль полного отчёта настроен.\n\n"
            f"Магазин: {store_name}\n"
            f"chat_id: {value}"
        )
